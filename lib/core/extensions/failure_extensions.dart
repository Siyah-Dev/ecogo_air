...
extension FailureHandling on Object {
  Either<Failure, T> toFailure<T>() {
    if (this is NetworkExceptions) {
      ...
      return Left(
        SocketFailure(_mapSocketError((this as SocketException).message)),
      );
      ...
      return const Left(
        UnknownFailure("Unexpected error occurred. Please try again."),
      );
    }
  }
}
...
String _mapNetworkError(String rawMessage) {
  if (rawMessage.contains("No route to host") ||
...
String _mapSocketError(String rawMessage) {
  if (rawMessage.contains("No route to host") ||
    ...
    return "Connection timed out. Please try again.";
  } else if (rawMessage.contains("Connection refused") ||
      rawMessage.contains("OS Error")) {
